...
| Windows client | [`syslog-pusher/`](syslog-pusher/) — `dist/SyslogPusher-0.9.2.exe` |
...
**Unified intelligence (v0.9.2)** — cross-source search, multiplex live stream, EventStore schema v2 (`analyses` + `meta_summaries`), runtime `config/sources.yaml`, journal window LLM, single alert engine on `events.sqlite`. Migrate syslogb alert rules: `scripts/migrate-alert-rules.py`.

**Syslog Pusher (Windows)** — [`syslog-pusher/`](syslog-pusher/) forwards Windows event logs and watched log files to the hub (or rsyslog). Pre-built installer: `syslog-pusher/dist/SyslogPusher-0.9.2.exe`.
...
1. Copy **`syslog-pusher/dist/SyslogPusher-0.9.2.exe`** to the server (or build from source — see [syslog-pusher/README.md](syslog-pusher/README.md)).
...
Output: `syslog-pusher\dist\SyslogPusher-0.9.2.exe` (single-file, win-x64, self-contained).
...
syslog-pusher/       Windows client (C#) — source + dist/SyslogPusher-0.9.2.exe
...
5. Deploy Windows clients from `syslog-pusher/dist/SyslogPusher-0.9.2.exe`
